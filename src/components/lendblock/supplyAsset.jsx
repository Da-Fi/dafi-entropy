...
const styles=theme => ({
  ...
  accordion: {
    ...
    borderRadius: '50px',
    borderBottom: '1px solid '+colors.dafiDefaulthex,
    "&:last-child": {
      borderBottom: 'none'
    }
  },
  ...
  MuiTypography: {
    h4: {
      colorPrimary: colors.dafiDefaulthex
  ...
  assetSummary: {
    display: 'flex',
    background: colors.dafident,
    border: '6px solid '+colors.dafident,
    borderRadius: '50px',
    alignItems: 'center',
    flex: 1,
    padding: '12px',
  ...
  assetActions: {
    ...
    border: '2x solid '+colors.dafiDefaulthex
  },
  removePadding: {
    padding: '0px'
  },
...
class SupplyAsset extends Component {
        ...
        >
          <div className={classes.assetSummary}>
            ...
            <div className={classes.headingAPY}>
              <Typography variant={'h4'} noWrap align='right'>{asset.supplyAPY? new BigNumber(asset.supplyAPY).toFixed(2,BigNumber.ROUND_DOWN):'0.00'}%</Typography>
            </div>
            <div className={classes.heading}>
              <Typography variant={'h4'} noWrap align='right'>{asset.balance? new BigNumber(asset.balance).toFixed(2,BigNumber.ROUND_DOWN):'0.00'} {asset.symbol}</Typography>
            </div>
            <div className={classes.heading}>
              <Typography variant={'h4'} noWrap align='right'>{asset.supplyBalance? new BigNumber(asset.supplyBalance).toFixed(2,BigNumber.ROUND_DOWN):'0.00'} {asset.symbol}</Typography>
            </div>
          </div>
        </AccordionSummary>
        ...
        <div className={classes.assetInfo}>
          ...
          <div className={classes.infoField}>
            ...
            <div className={classes.flexy}>
              <Typography variant={'h4'} noWrap>$ {limit? limit.toFixed(2):'0.00'}</Typography>
            </div>
          </div>
          <div className={classes.infoField}>
            <Typography variant={'h5'} className={classes.grey}>Borrow limit used:</Typography>
            <div className={classes.flexy}>
              <Typography variant={'h4'} noWrap>{theLimitUsed.toFixed(2)}%</Typography>
            </div>
          </div>
          <div>
          ...
          <div className={classes.tradeContainer}>
            ...
            <div className={classes.buttons}>
              <Button
                ...
                variant="contained"
                color="primary"
                disabled={loading||!supplyAmount||new BigNumber(supplyAmount).gt(new BigNumber(asset.balance))}
                onClick={this.onSupply}
                fullWidth
              >
                <Typography className={classes.buttonText} variant={'h5'}>
                  {asset.balance===0&&'No Balance to Supply'}
                  {asset.balance>0&&supplyAmount&&new BigNumber(supplyAmount).gt(new BigNumber(asset.balance))&&'Insufficient Balance'}
                  {asset.balance>0&&(!supplyAmount||new BigNumber(supplyAmount).lte(new BigNumber(asset.balance)))&&'Supply'}
                </Typography>
              </Button>
            </div>
          </div>
            ...
            <div className={classes.buttons}>
              ...
              >
                <Typography className={classes.buttonText} variant={'h5'} >
                  ...
                  {asset.supplyBalance>0&&new BigNumber(withdrawAmount).gt(new BigNumber(asset.supplyBalance))&&'Insufficient Balance'}
                  {asset.supplyBalance>0&&(!withdrawAmount||new BigNumber(withdrawAmount).lte(new BigNumber(asset.supplyBalance)))&&'Withdraw'}
                </Typography>
              </Button>
            </div>
          </div>
  ...
  setSupplyAmount=(percent) => {
    ...
    const amount=new BigNumber(asset.balance).times(percent).div(100).toFixed(asset.decimals)
  ...
  setWithdrawAmount=(percent) => {
    ...
    const amount=new BigNumber(asset.supplyBalance).times(percent).div(100).toFixed(asset.decimals)
